################################################################################
# NumberPartitioningNoRisp_DWaveSam.py
#
# Classroom companion for the second Number Partitioning example in SiSi2Qubo.tex.
#
...
#   exists. In this specific example the absence of zero-energy states is known
#   mathematically, because the total weight V = 5 is odd.
################################################################################
...
print('## Instance with no zero-energy answer\n')
...
def print_best_states(sampleset, variable_names):
    ...
    For this instance, the mathematical reason why no energy-0 state exists is
    the odd total weight V = 5. The QPU run should therefore return states with
    positive energy. However, the sampler output is still an experimental result:
    it is not the proof. The proof is the parity argument.
    """
    ...
    if best_energy == 0:
        print('Interpretation: the run found an energy-0 state.')
        print('This would contradict the parity argument, so the model/output')
        print('should be inspected carefully.')
    else:
        print('Interpretation: the run found no zero-energy state.')
        print('Here this agrees with the mathematical parity argument:')
        print('the total weight is odd, so two identical integer sums are impossible.')
...
if __name__ == '__main__':
    ...
    # Since V is odd, no subset can have the same sum as its complement.
    # Therefore the Hamiltonian cannot reach energy 0; the minimum energy is 1.
    #
    # In this case, the D-Wave run is not used as a proof of non-existence. It is
    # used as a hardware sampling experiment whose best states should be compared
    # with the mathematical parity argument.
    ############################################################################
    run_instance(
